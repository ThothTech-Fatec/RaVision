...
public interface BaseVendasRepository extends JpaRepository<BaseVendas, Long> {
    ...
    void deleteByDateRefBetween(LocalDate startDate, LocalDate endDate);
    List<BaseVendas> findByDateRef(LocalDate dateRef);

    @org.springframework.data.jpa.repository.Query("SELECT SUM(v.vlrVenda) FROM BaseVendas v WHERE v.dateRef BETWEEN :startDate AND :endDate")
    java.math.BigDecimal sumarizarFaturamentoGeral(@org.springframework.data.repository.query.Param("startDate") LocalDate startDate, @org.springframework.data.repository.query.Param("endDate") LocalDate endDate);
    ...
    @org.springframework.data.jpa.repository.Query("SELECT new br.com.ravision.backend.dto.DashboardAgregacaoDTO(v.descrMarca, SUM(v.vlrVenda)) " +
           "FROM BaseVendas v WHERE v.dateRef BETWEEN :startDate AND :endDate GROUP BY v.descrMarca ORDER BY SUM(v.vlrVenda) DESC")
    List<br.com.ravision.backend.dto.DashboardAgregacaoDTO> sumarizarFaturamentoPorMarca(@org.springframework.data.repository.query.Param("startDate") LocalDate startDate, @org.springframework.data.repository.query.Param("endDate") LocalDate endDate);
